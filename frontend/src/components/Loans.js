...
const Loans = () => {
  ...
  const [formData, setFormData] = useState({ lead: '', amount: '', status: 'pending', notes: '' });
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const [loanRes, leadRes] = await Promise.all([api.getLoans(), api.getLeads()]);
      const loanData = Array.isArray(loanRes.data) ? loanRes.data : (loanRes.data?.results || []);
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      setFormData({ lead: '', amount: '', status: 'pending', notes: '' });
      fetchData();
    } catch (err) {
  ...
  const handleStatusChange = async (loan, status) => {
    try {
      await api.updateLoan(loan.id, { status });
      fetchData();
    } catch (err) {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50">
      <div className="px-4 sm:px-6 lg:px-8 py-6 sm:py-8 bg-white border-b border-gray-200">
        ...
        <p className="text-sm sm:text-base text-gray-600 mt-1 sm:mt-2">Track loan status and progress</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Add Loan</h2>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-100 border-b border-gray-200">
                  ...
                  <th className="px-4 sm:px-6 py-3 sm:py-4 text-left text-xs sm:text-sm font-semibold text-gray-800">Amount</th>
              ...
              <tbody>
                {loans.map((loan, index) => (
                  <tr key={loan.id} className={index % 2 === 0 ? 'bg-white border-b' : 'bg-gray-50 border-b'}>
                    <td className="px-4 sm:px-6 py-3 sm:py-4 text-xs sm:text-sm text-gray-900">{loan.lead_id} - {loan.lead_name}</td>
                    <td className="px-4 sm:px-6 py-3 sm:py-4 text-xs sm:text-sm text-gray-700">{loan.amount || '-'}</td>
                    ...
                    <td className="px-4 sm:px-6 py-3 sm:py-4 text-xs sm:text-sm text-gray-700">{loan.notes || '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          {loans.length === 0 && <div className="text-center py-8 text-gray-500">No loans added yet.</div>}
        </div>
      </div>
    </div>
  );
